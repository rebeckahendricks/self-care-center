...
function handleDragOver(e) {
    ...
    if (target) {
        ...
        if (e.clientY < target.getBoundingClientRect().top + target.getBoundingClientRect().height / 2) {
            container.insertBefore(placeholder, target); // Place placeholder before the target
        } else {
            container.insertBefore(placeholder, target.nextSibling); // Place placeholder after the target
        }
    }
}
...
function handleDragEnter(e) {
    ...
    if (target && !target.classList.contains('dragging')) {
        // Apply a visual shift to all cards below the target
        let found = false;
        ...
        cards.forEach(card => {
            ...
            // Start shifting cards after the target
            if (found) {
...
function handleDrop(e) {
    ...
    placeholder.replaceWith(sourceElement); // Replace the placeholder with the source element
    sourceElement.style.opacity = '';
}

function handleDragEnd(e) {
    placeholder.remove(); // Remove the placeholder when dragging ends
    this.style.opacity = '';
}
